...
toggle_signature_check()
{
    ...
    ui_print("签名验证: %s\n", signature_check_enabled ? "已启用" : "禁用");
}
...
{
    static char* headers[] = {  "从SD卡选择ZIP刷机包安装",
                                "",
                                NULL
    };
...
int confirm_selection(const char* title, const char* confirm)
{
    ...
    char* confirm_headers[]  = {  title, "该操作无法恢复.", "", NULL };
...
void show_partition_menu()
{
    ...
    for (;;)
    {
        ...
        if (!is_data_media()) {
          options[mountable_volumes + formatable_volumes] = "挂载USB存储设备";
...
void show_nandroid_advanced_restore_menu(const char* path)
{
    ...
    static char* advancedheaders[] = {  "选择一个镜像恢复",
                                "",
                                NULL
    };
...
static void choose_default_backup_format() {
    static char* headers[] = {  "选择备份格式",
                                "",
                                NULL
    };

    char **list;
    char* list_tar_default[] = { "tar (已选择)",
        "dup",
        NULL
    };
...
static void partition_sdcard(const char* volume) {
    ...
    if (0 == __system(cmd))
        ...
        ui_print("你的SD卡进行分区时发生错误。请有关详细信息，请参阅tmp/recovery.log\n");
}
    ...
    for (;;)
    {
        ...
        switch (chosen_item)
        {
            ...
            case 1:
                ...
                if (confirm_selection( "确认清除?", "是的 - 清除Dalvik缓存")) {
...
void handle_failure(int ret)
{
    ...
    ui_print("/tmp/recovery.log 已复制到/sdcard/clockworkmod/recovery.log.请打开ROM Manager来报告问题.\n");
}
